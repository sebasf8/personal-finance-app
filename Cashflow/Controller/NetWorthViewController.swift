...
class NetWorthViewController: UIViewController {
    ...
    private func loadData() {
        totalNetworth.text = NumberFormatters.currencyFormat(number: NSNumber(value: networth.getResult()), locale: Locale.current)
    ...
    @IBAction func unwind(_ sender: UIStoryboardSegue) {
        ...
        } else if let sourceViewController = sender.source as? AddStatementItemViewController, let statementItem = sourceViewController.statementItem {
            let newIndexPath: IndexPath!

            if statementItem.type == .asset {
                newIndexPath = IndexPath(row: networth.assets.count, section: 0)
                try? networth.addAsset(asset: statementItem)
            } else if statementItem.type == .liability {
                newIndexPath = IndexPath(row: networth.liabilities.count, section: 1)
                try? networth.addLiability(liability: statementItem)
            } else {
...
extension NetWorthViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.ammountLabel.text = NumberFormatters.currencyFormat(number: statementItem.amount as NSNumber, locale: Locale.current)
