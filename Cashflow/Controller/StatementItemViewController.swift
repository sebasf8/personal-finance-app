...
class StatementItemViewController: UIViewController {
    ...
    private func setupStatementItemDetail() {
        let amount = NumberFormatters.currencyFormat(number: statementItem!.amount as NSNumber, locale: Locale.current)
    ...
    private func setupStatementItemHistoryTable() {
        let cellView = UINib(nibName: "HistoryTableViewCell", bundle: nil)
        history = statementItem!.history?.allObjects as? [StatementItemMeasure]
        history?.sort(by: {$0.date! > $1.date!})

        historyTableView.delegate = self
        historyTableView.dataSource = self
        historyTableView.register(cellView, forCellReuseIdentifier: "historyCell")
    }
    ...
    private func saveHistoryItem(ammount: Double) {
        ...
        self.statementItem?.amount = ammount
        do {
        ...
        self.amountLabel.text = NumberFormatters.currencyFormat(number: ammount as NSNumber, locale: Locale.current)
    }
}
...
extension StatementItemViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let amount = NumberFormatters.currencyFormat(number: historyItem.ammount as NSNumber, locale: Locale.current)
        let date = DateFormatter.localizedString(from: historyItem.date!, dateStyle: .short, timeStyle: .short)
        cell?.ammountLabel.text = amount
